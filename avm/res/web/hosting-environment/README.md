...
| `Microsoft.Web/hostingEnvironments` | [2023-01-01](https://learn.microsoft.com/en-us/azure/templates/Microsoft.Web/hostingEnvironments) |
...
This instance deploys the module with a base set of parameters.
...
module hostingEnvironment 'br/public:avm/res/web/hosting-environment:<version>' = {
  name: '${uniqueString(deployment().name, location)}-test-whemin'
  params: {
    // Required parameters
    name: 'whemin001'
...
```bicep
module hostingEnvironment 'br/public:avm/res/web/hosting-environment:<version>' = {
  name: '${uniqueString(deployment().name, location)}-test-whemax'
  params: {
    // Required parameters
    name: 'whemax001'
    subnetResourceId: '<subnetResourceId>'
...
```bicep
module hostingEnvironment 'br/public:avm/res/web/hosting-environment:<version>' = {
  name: '${uniqueString(deployment().name, location)}-test-whewaf'
  params: {
    // Required parameters
    name: 'whewaf001'
    subnetResourceId: '<subnetResourceId>'
...
| [`dedicatedHostCount`](#parameter-dedicatedhostcount) | int | The Dedicated Host Count. By default zoneRedundant is true, this this value must be "-1". If you want zonal physical hardware isolation enabled, set to 2, if not, use 0. |
...
The Dedicated Host Count. By default zoneRedundant is true, this this value must be "-1". If you want zonal physical hardware isolation enabled, set to 2, if not, use 0.
